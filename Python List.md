...
Variables in Python are references (pointers) to objects. When you assign b = a, you are copying the reference, not the data. Both variables point to the same memory address.
...
Detailed Explanation:
append increases length by exactly 1. extend increases length by len(iterable).
Time & Space Complexity:
...
x = [1, 2]
x.append([3, 4])
print(f"Append result: {x}") # [1, 2, [3, 4]]

# Scenario B: Extend
y = [1, 2]
y.extend([3, 4])
print(f"Extend result: {y}") # [1, 2, 3, 4]
...
val = lst.pop(1)      # Removes index 1 (20), returns 20
print(f"Popped: {val}, List: {lst}")

lst.remove(30)        # Removes value 30
print(f"After remove: {lst}")

del lst[0]            # Removes index 0 (10)
...
Why Interviewers Ask This: Critical for performance awareness. Using in inside a loop can inadvertently create O(N^2) logic.
Comparisons:
 * List in: O(N)
 * Set in: O(1) (Average)
Sample Code:
...
start = time.time()
# Worst case: searching for element at the end
...
print(f"a is b? {a is b}") # False (Different IDs)
...
print(lst[-1]) # 'c'
print(lst[-3]) # 'a'
# Internal logic roughly: lst[len(lst) + (-1)]
...
[B, C, D]
 ^
Step 2: Loop moves to index 1.
[B, C, D]
    ^
Result: B was skipped!
...
for item in lst:
    if item % 2 == 0:
        lst.remove(item)
# Result might be [1, 3, 4] or similar incorrect state

# Good Approach (Iterate over copy)
lst = [1, 2, 3, 4]
for item in lst[:]:
    if item % 2 == 0:
        lst.remove(item)
...
Detailed Explanation: This ensures that append has an amortized time complexity of O(1). If it resized every time, complexity would be O(N^2) for building a list.
...
for i in range(5):
    lst.append(i)
    # You will see size jump in steps, not linearly every time
...
# New list created
s_nums = sorted(nums)
print(nums)   # [3, 1, 2] (Original intact)
...
print(x) 
# Output: "global" (Python 3)
# Output: 4 (Python 2 - leaked)
...
# List Comprehension (Allocates memory for all numbers)
lc = [x for x in range(10000)]
print(sys.getsizeof(lc)) # ~87KB

# Generator Expression (Allocates definition only)
ge = (x for x in range(10000))
print(sys.getsizeof(ge)) # ~100-200 Bytes
...
def move_zeros(nums):
    ...
    # No second pass needed with swap logic above.
    # If using overwrite logic, fill nums[pos:] with 0s.
...
# If Sets are forbidden (Logic Approach):
...
Complexity: O(N \cdot K \log K) where N is list length and K is max string length.
...
def product_except_self(nums):
    ...
    # Left Pass: Calculate product of all to the left
    left_product = 1
...
Explanation: We use prefix and suffix products.
 * answer array stores prefix products.
 * We iterate backwards maintaining a running right_product and multiply it into answer.
...
Why Interviewers Ask This: Tests Prefix Sum + Hashing. O(N^2) brute force is easy, but O(N) requires this specific technique.
